...
Use Node.js 22 e pnpm 11.19.0 (versão registrada em package.json).
...
`data/` → unidades, fontes, parâmetros e observações. `physics/` → potenciais, estados, leapfrog e invariantes, sem dependências gráficas. `simulation/` → relógio de apresentação. `visualization/` → escolhas decorativas. `entities/`, `graphics/`, `shaders/` → GPU/câmeras. `ui/` → gráficos, controles e exportação. `tests/fixtures/legacy.ts` é somente arquivo histórico para auditoria, nunca importado pela aplicação.

## Deploy e recuperação

O projeto continua na raiz do repositório, entrada `index.html`, código em `src/`, saída em `dist/`. `vercel.json` declara Vite, `pnpm build` e `dist`; não requer backend, variáveis de ambiente ou API paga. Configure Node 22 no projeto Vercel caso esteja fixado em versão incompatível com pnpm. A configuração real do painel Vercel ainda deve ser conferida durante a publicação.
