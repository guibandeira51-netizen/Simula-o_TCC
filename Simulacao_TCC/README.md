...
- [Modelo de três componentes e relatório desta revisão](docs/MODELO-TCC.md).
...
O modelo contém exatamente bojo Hernquist, disco Miyamoto–Nagai e halo NFW. Os parâmetros preservados têm procedência no Gala v1, mas esta seleção não reproduz o ajuste integral desse benchmark. Modelo estático e axisimétrico: sem barra/espirais gravitacionais, gás, colisões, auto-gravidade ou cosmologia. Visuais não são catálogo de estrelas nem transporte radiativo. Resultados numéricos precisos validam a implementação desse modelo, não provam que ele descreve todos os aspectos da Galáxia real.
