...
function start() {
  ...
  function telemetry() {
    ...
    el("vector-readout").innerHTML = [
      ...
      ...(["disk", "bulge", "nucleus", "halo", "total"] as const).map((key) => [
      ...
      .join("");
    const primarySummary = primaryTrajectory.summary();
    const comparisonSummary = comparisonTrajectory.summary();
    el("primary-scenario-label").textContent = primary.field.components.halo
      ? "Com halo"
      : "Campo selecionado · sem halo";
  ...
  el("btn-export").onclick = () =>
    downloadJSON({
      ...
      trajectory: primaryTrajectory.summary(),
      comparisonTrajectory: comparisonTrajectory.summary(),
