...
export class TrajectoryRecorder {
  ...
  summary(): TrajectorySummary {
    ...
    return {
      rMin,
      rMax,
      pericenter: this.pericenters.length ? Math.min(...this.pericenters) : rMin,
      apocenter: this.apocenters.length ? Math.max(...this.apocenters) : rMax,
      eccentricity: rMax + rMin > 0 ? (rMax - rMin) / (rMax + rMin) : 0,
      meanSpeed: this.count ? this.speedSum / this.count : 0,
      radialPeriod: mean(this.radialPeriods),
      azimuthalPeriod: mean(this.azimuthalPeriods),
