...
test("Componentes de aceleração somam exatamente o vetor total", () => {
  ...
  for (const q of [[0.7, -0.4, 0.2], [8.178, 1.1, -0.3], [30, -5, 4]] as Vec3[]) {
    ...
    const sum = (i: number) => c.disk[i] + c.bulge[i] + c.nucleus[i] + c.halo[i];
    for (let i = 0; i < 3; i++) near(c.total[i], sum(i), 1e-12);
  ...
  for (let i = 0; i < 3; i++)
    near(c.total[i], c.disk[i] + c.bulge[i] + c.nucleus[i], 1e-12);
});
