...
import { G, MODEL, SOLAR, VELOCITY, ACCELERATION } from "../src/data/parameters";
import { OBSERVATIONS } from "../src/data/observations";
import { PhysicsEngine as Legacy } from "./fixtures/legacy";
import { initialDiagnostics } from "../src/simulation/comparison";
...
const solar = [true, false].map((halo) => {
  ...
  return {
    halo,
    initial: initialDiagnostics(initial, field),
    trajectory: trajectory.summary(m.baseline.E < 0),
    maxRelativeEnergy: m.maxRelativeEnergy,
    maxRelativeLz: m.maxRelativeAngular,
    durationGyr: s.t,
    maxEnergy: m.maxEnergy,
    maxLz: m.maxAngular,
    rmin,
...
const constraints = OBSERVATIONS.map((o) => ({
  R: o.radiusKpc,
  observed: o.velocityKmS,
  model: full.circularSpeed(o.radiusKpc),
  difference: full.circularSpeed(o.radiusKpc) - o.velocityKmS,
  source: o.source.url,
  note: o.note,
}));
const data = {
  ...
  constraints,
  consistency: {
    maxAccelerationRelativeResidual: Math.max(...[0.5, 1, SOLAR.R, 20, 50].map(R => {
      const c = full.accelerationComponents([R, -0.2 * R, 0.1 * R]);
      return Math.hypot(...c.total.map((v,i) => v-c.disk[i]-c.bulge[i]-c.halo[i])) / Math.hypot(...c.total);
    })),
    maxCircularSquaredRelativeResidual: Math.max(...[0.5, 1, SOLAR.R, 20, 50].map(R =>
      Math.abs(full.circularSpeed(R) ** 2 + R * full.acceleration([R,0,0])[0] / ACCELERATION) / full.circularSpeed(R) ** 2)),
  },
};
...
for (let R = 0.5; R <= 50; R += 0.25)
  rows.push(
    [
      ...
      Math.sqrt(
        (G * (MODEL.disk.mass + MODEL.bulge.mass)) / R,
      ),
    ].join(","),
  );
