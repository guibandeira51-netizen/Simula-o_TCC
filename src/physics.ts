// Implementação rigorosa da física do TCC baseada no Python original
export class PhysicsEngine {
    ...
    // Parâmetros do Halo (NFW)
    rs = 16.0;
    rho0 = 0.01430 * 1e9;

    // Parâmetros do Disco (Miyamoto-Nagai)
    Sigma0 = 500 * 1e6;
    Rd = 3.0;
    ...
    haloMassEnclosed(r: number) {
        let x = r / this.rs;
        return 4 * Math.PI * this.rho0 * Math.pow(this.rs, 3) * (Math.log(1 + x) - x / (1 + x));
    }

    vHalo(r: number) {
        r = Math.max(r, this.EPS);
        return Math.sqrt(this.G * this.haloMassEnclosed(r) / r);
    }

    vBulge(r: number) {
        r = Math.max(r, this.EPS);
        return Math.sqrt(this.G * this.Mb * r / Math.pow(r + this.a, 2));
    }

    vDisk(r: number) {
        r = Math.max(r, this.EPS);
        return Math.sqrt(this.G * this.Md * r * r / Math.pow(r * r + Math.pow(this.ad + this.bd, 2), 1.5));
    }

    vTotal(r: number) {
        return Math.sqrt(Math.pow(this.vHalo(r), 2) + Math.pow(this.vDisk(r), 2) + Math.pow(this.vBulge(r), 2));
    }
    ...
    accDisk(x: number, y: number, z: number) {
        ...
        let az = (z_term < this.EPS) ? 0.0 : -this.G * this.Md * B * z / (z_term * denom);
        return {ax, ay, az};
    }
    ...
    potHalo(r: number) {
        ...
        return - (4 * Math.PI * this.G * this.rho0 * Math.pow(this.rs, 3) * Math.log(1 + r/this.rs)) / r;
    }
    potDisk(R: number, z: number) {
        if (!this.useDisk) return 0;
        return - (this.G * this.Md) / Math.sqrt(R*R + Math.pow(this.ad + Math.sqrt(z*z + this.bd*this.bd), 2));
    }
    potBulge(r: number) {
    ...
    getEnergies() {
        // Ek e Ep em (km/s)^2 para padronização astrofísica
        let v2 = (this.vx*this.vx + this.vy*this.vy + this.vz*this.vz) / (this.KM_S_TO_KPC_GYR * this.KM_S_TO_KPC_GYR);
        let Ek = 0.5 * v2; 
    ...
    reset() {
        ...
        let vR = this.e_target * kappa0 * this.r0; // kpc/Gyr
    ...
    step(dt: number) {
        ...
        this.t += dt;
        return acc; // Retorna aceleração do último passo para UI
    }
}
