...
export class PhysicsEngine {
    ...
    vHalo(r: number) { return Math.sqrt(this.G * this.haloMassEnclosed(Math.max(r, this.EPS)) / Math.max(r, this.EPS)); }
    vBulge(r: number) { return Math.sqrt(this.G * this.Mb * Math.max(r, this.EPS) / Math.pow(Math.max(r, this.EPS) + this.a, 2)); }
    vDisk(r: number) { let rm = Math.max(r, this.EPS); return Math.sqrt(this.G * this.Md * rm * rm / Math.pow(rm * rm + Math.pow(this.ad + this.bd, 2), 1.5)); }
    ...
    reset() {
        ...
        this.useHalo = tempHalo; this.useDisk = tempDisk; this.useBulge = tempBulge;
    ...
    step(dt: number) {
        ...
        this.vx += 0.5 * acc.ax * dt; this.vy += 0.5 * acc.ay * dt; this.vz += 0.5 * acc.az * dt;
