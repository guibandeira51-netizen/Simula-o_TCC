...
export class UIManager {
    ...
    physCompare: PhysicsEngine; // Usado para o painel da direita no split screen
    ...
    constructor(phys1: PhysicsEngine, phys2: PhysicsEngine) {
        ...
        this.btnOrbit.addEventListener('click', () => {
            ...
            this.btnOrbit.classList.add('active'); this.btnSun.classList.remove('active');
        });

        this.btnSun.addEventListener('click', () => {
            this.cameraMode = 'sun';
            this.btnSun.classList.add('active'); this.btnOrbit.classList.remove('active');
        });
    ...
    updateTelemetry(accMag: number) {
        let r = Math.sqrt(this.physMain.x**2 + this.physMain.y**2 + this.physMain.z**2);
        let v = Math.sqrt(this.physMain.vx**2 + this.physMain.vy**2 + this.physMain.vz**2) / this.physMain.KM_S_TO_KPC_GYR;
        let { Ek, Ep } = this.physMain.getEnergies();

        this.elTime.innerHTML = `${this.physMain.t.toFixed(4)} <small>Gyr</small>`;
        this.elRadius.innerHTML = `${r.toFixed(2)} <small>kpc</small>`;
        this.elVel.innerHTML = `${v.toFixed(2)} <small>km/s</small>`;
        this.elAcc.innerHTML = `${accMag.toExponential(2)} <small>kpc/G²</small>`;
        
        let sk = Ek.toExponential(2);
        let sp = Ep.toExponential(2);
        this.elKin.innerText = sk;
        this.elPot.innerText = sp;

        // Atualiza gráfico a cada X ms indiretamente pela chamada do main loop controlada
    }
}
